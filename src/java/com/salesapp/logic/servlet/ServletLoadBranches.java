 ...
 */
@WebServlet(name = "branches", urlPatterns = {"/branches"})
public class ServletLoadBranches extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet LoadBranches</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet LoadBranches at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void processDoGet(HttpServletRequest request, HttpServletResponse response)  throws ServletException, IOException{
        ArrayList<Branch> branches = (ArrayList<Branch>) BranchController.findAll();
        request.getSession().setAttribute("branches", branches);
        getServletConfig().getServletContext().getRequestDispatcher("/templates/branches/loadBranches.jsp").forward(request,response);
    ...
    protected void processLoadFiled(HttpServletRequest request, HttpServletResponse response)  throws ServletException, IOException{
      ...
      getServletConfig().getServletContext().getRequestDispatcher("/templates/branches/prevDataBranches.jsp").forward(request,response);
